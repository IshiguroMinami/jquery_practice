...
done()およびfail()内で関数を実行している際、引数を渡していないにも関わらず、それぞれの関数が引数を受け取れるのは
これがjQueryのAjaxメソッドの仕組みに基づいて動作しているからです。

done()とfail()はjQueryのDeferredオブジェクトのメソッドになります。
Ajaxリクエストの結果が戻ってきた際に、登録したコールバック関数が実行されます。
この際、jQueryは成功時または失敗時のデータを自動的にコールバック関数に渡します。

.done()や.fail()ではコールバック関数に必要な引数をjQueryが内部的に渡してくれるため、引数を渡す記述が不要です。
この仕組みにより、簡潔かつ柔軟に非同期処理を扱えるようになっています。

//done()の場合
・Ajax リクエストが成功すると、サーバーから返されたデータ（レスポンス）が response に渡されます。
・この response は $.ajax() メソッドが内部的に取得したデータで、コールバック関数に引数として自動的に供給されます

//fail()の場合
・リクエストが失敗した際に、エラーオブジェクトが err に渡されます。
・この err は、失敗の理由やステータスコードなどの情報を含んでいます。
*/

/*なぜそのように実装したのか説明してください。*/
//Ajax 成功時や失敗時の処理に集中することができ、コードの再利用性や可読性が向上します。
